...
Read this when you write the prose sections of a spec or a generated doc. In an onboard run that means the prose in spec.md (Current State, Architecture overview, Gotchas) and in CLAUDE.md (Project Identity). The goal is one linear read, with no backtracking.

**The principle.** Every time the reader holds a clause open while reading ahead, you spend their working memory. Close each loop as fast as you can. Every rule below is one way to close a loop sooner.
...
- **Put it in positive form.** Say what is, not what isn't. "The parser rejects empty rows," not "the parser does not accept rows that aren't populated." A negation makes the reader compute "not X" and hold it; the positive form lands directly. A section that is deliberately a prohibition list is the exception.
...
- **Keep parallel items parallel.** Co-ordinate ideas take the same grammatical shape — one consistent form for every item in a list (the same shape for each component line, each "Not yet implemented" entry). The reader learns the pattern once, then reads the rest on autopilot instead of re-parsing each item.

---

## What this does NOT touch

This shapes sentences and layout only. It does not loosen the density rules the spec and CLAUDE.md already follow — semantic density, named lists, exact commands, the "Not yet implemented" list. Keep all the precision (names, numbers, versions). Apply the style to the prose around that precision, never to the precision itself.
