...
description: Split a spec into an optimal set of implementation tasks. Balances three constraints — MR review size, parallel execution, and per-task context size. Writes the task breakdown into the spec's Tasks section and reports the waves, trade-offs, and flags. Decomposes and sequences; does not implement.
