...
Every check you write to prove a slice must hold these properties. They are what make "done" mean built and proven, not built and asserted. `check` (the fresh-context Gate B) hunts the fakes below from a clean context; ship one and it comes back. Cheaper to refuse it here.
...
Build the checks from the acceptance criteria and verification plan in the intent `scoping` wrote. Don't invent new criteria here; if the intent is missing one you need, that's a gap to raise, not to author.
...
- **Loosening or deleting a failing check to reach green.** Relaxing a bound, widening a tolerance, skipping the case, catching the exception the check should see. That fakes the result. Fix the code, not the check.
