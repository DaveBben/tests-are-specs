...
Dispatch a single `generic-code-reviewer` agent to perform a rapid review of code changes.
Covers all four review dimensions in one pass, focusing only on findings that must be
fixed.
...
1. Determine the diff base (same logic as /deep-review)
2. Launch a single `generic-code-reviewer` agent with:
   ```
   Review the code changes against [base reference]. [Base reference] is the base branch
   or commit to diff against.
   ```
3. Return the agent's report directly — no consolidation needed (single agent)
