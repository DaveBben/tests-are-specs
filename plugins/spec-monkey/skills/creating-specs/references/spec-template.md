<!--
A SPEC IS A FOLDER OF THREE DOCUMENTS, ONE PER READER. This reference lays out every file to
create. Each block below marked `FILE: <path>` is a separate file, delimited by an HTML comment.
Create each at its path under docs/specs/{slug}/:
...
WHY THIS SPLIT: the readers want different slices, not different amounts. The human reviewer wants
the judgment layer — decisions, risks, accepted gaps — so spec.md carries it, short and standalone.
The implementer and the auditor want the mechanical layer — requirements, contracts, commands, the
worked case — so contract.md carries all of it in one read, with no review-time material diluting
their context. The reasoning that justified the design — current-state facts, the failure-mode
lenses, impact — matters most before approval, so evidence.md carries it, opened on doubt and
rarely after approval. Keep spec.md readable on its own; give each detail file a heading and a
backlink so it stands alone too.

THE SECTION SET IS THE SCHEMA: every canonical section below must exist in its fixed home file,
under its canonical heading — or, for a folded block (Coverage exceptions, Worked case,
Contingencies), its canonical `**bold**` key. A detail section with nothing to say gets a justified
`N/A — reason`, never silence; only the brief subsections marked as omittable may be dropped. Cite
a section by its heading or bold key, never by a number or a position.
...
SINGLE SOURCE OF TRUTH: each fact lives in ONE section; everywhere else references it by ID
(FR-001, SC-001), by section heading, or by a Markdown link. A one-line headline in the brief with
the detail in a detail/ file is disclosure, not duplication — but the same sentence in two places
...
one exception is verification: the run commands that prove success are concrete (*Verification
...
---
spec_monkey: "1.2.0"             # spec-monkey format version; also marks this as a spec-monkey spec
...
<!-- ONLY the questions that gate the release. The rest live under *Open questions & assumptions*
     in detail/evidence.md. Omit if none. -->
- **Open (blocking):** <question>; gates <what it blocks>.

### Blast radius & reversibility
<!-- One line each. The full touch-list is *Who & what this touches* (detail/evidence.md); the
     backout detail is *When it happens* (detail/contract.md). -->
...
<!-- The CONDITION under which this turns on ("ship only after N", "dark-launch, enable when M holds").
     The mechanics (flag name, command sequence) are HOW → *When it happens* in detail/contract.md. -->

## Contents
<!-- The reading contract. Keep the promise lines below in the composed spec — they route each
     reader. Keep the section lists so a reader routes without opening both files. -->

**How to read this spec:** the approver can sign off from the brief above alone, opening the files
below on doubt. The implementer and the auditor load the contract whole. A deep reviewer reads
everything.

- [The contract](detail/contract.md) — what the implementer builds and the auditor checks:
  *Requirements & success criteria* · *Constraints & non-functional bounds* · *Data & interface
  contract* · *When it happens* · *Out of scope* · *Known limitations & honest gaps* (incl. coverage
  exceptions) · *Verification approach & commands* (incl. the worked case)
...
<!-- The build contract: everything that binds — what the implementer builds against and the
     auditor checks against. Review-time reasoning goes in evidence.md, not here — this file is
     loaded whole into an implementing agent's context. -->
...
<!-- One FR = one independently verifiable obligation. Name the actor and the action; one condition
     ...
     leading **WHEN**; cross-change sequencing goes under *When it happens*. Any guarantee word
...
<!-- Mark as "N/A — reason" if nothing crosses a boundary. ONLY externally observable contracts this
...
<!-- Part of the contract — never bury it. What a passing check still would NOT prove, and what
     ...
     rather than handles. Canonical home for accepted risk; *Failure modes* (evidence.md) references
     back here rather than restating. -->
- <admitted gap or accepted risk>: <what it leaves unproven, and what (if anything) covers it>

**Coverage exceptions**
<!-- The final-sweep artifact. Every FR has an SC beside it under *Requirements & success criteria*,
     or is listed here as a declared gap with a reason. A behavior an FR marks "must preserve" gets
...
<!-- The review-time reasoning: what a deep reviewer needs to judge the design, and what the author
     needed to reach it. After approval this file is rarely opened — nothing that binds may live
     only here. A risk decision recorded in *Failure modes* must land in its contract home (an FR,
     *Known limitations & honest gaps*, or *Out of scope*); this file keeps the per-risk reasoning,
     and the auditor reads it as the index when tracing those decisions. -->
...
<!-- Work EVERY lens. Each risk gets a decision: HANDLE (→ an FR), ACCEPT (→ Known limitations &
     ...
     none apply". Cite the FR a handled risk relates to. The **Contingencies** block at the end is
     the exception: fallbacks carry a trigger, not a decision, and bind nothing. -->
...
<!-- Conditional fallbacks that fire ONLY if a trigger hits: not something the release must satisfy
     ...
     trigger. Mark as "N/A — reason" if none. -->
...
1. Layout: docs/specs/{slug}/spec.md plus docs/specs/{slug}/detail/contract.md and
   docs/specs/{slug}/detail/evidence.md. spec.md is the entry point and holds the frontmatter and
   the decision brief; contract.md holds the binding sections; evidence.md holds the review-time
   sections. (Specs written under format 1.0.0 spread the equivalent sections one-per-file under
   detail/; locate a section by its heading either way.)
...
3. Sections: the canonical section set is the schema. Each section lives in its fixed home file,
   under its canonical heading — or, for a folded block, its canonical `**bold**` key (convention
   5); the mapping is spec.md's Contents block. Cite a section by its heading or key, never a
   number.
4. IDs are the join keys: FR-NNN (one SHALL obligation) and SC-NNN (one verifiable outcome), unique
   across the spec. Each SC sits directly beneath the FR it verifies under *Requirements & success
   criteria*.
