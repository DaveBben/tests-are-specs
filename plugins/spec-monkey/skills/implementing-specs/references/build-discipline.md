...
- **Trace.** Every FR maps to a real change in the diff; every SC maps to a test that would flip to failing if the behavior broke. An FR with no corresponding change, or an SC whose test can't fail, is a hole.
